...
def получить_фото_блюда(запрос_блюда):
    """Получает URL изображения блюда с Unsplash"""
    try:
        url = f"https://source.unsplash.com/800x600/?{запрос_блюда}"
        return url
...
def главная():
    ...
    if тема.get('тип') == 'рецепт':
        запрос = тема.get('фото_запрос') or "healthy food"
        фото_url = получить_фото_блюда(запрос)
        if not фото_url:
            фото_url = random.choice(ФИТНЕС_ИЗОБРАЖЕНИЯ)
        print(f"🖼️ Фото для рецепта: {запрос}")
    else:
        фото_url = random.choice(ФИТНЕС_ИЗОБРАЖЕНИЯ)
