...
def _продукты_с_актуальными_ценами(продукты: list) -> list:
    """
    ...
    for строка in (продукты or []):
        ...
        if вилка:
            ...
            # последний фолбэк: оставляем как есть, но помечаем диапазоном на основе старой цены (x1.5–x2.5)
            pm = re.search(r"(\\d+)", строка)
            if pm:
                base = int(pm.group(1))
                lo = max(1, int(round(base * 1.5)))
                hi = max(lo, int(round(base * 2.5)))
                обновленные.append(f"{лев} — {_формат_вилки(lo, hi)}")
            else:
                обновленные.append(строка)
...
def сгенерировать_фитнес_пост(тема_данные):
    ...
    if тип == 'рецепт':
        ...
        тема_для_рецепта["продукты"] = _продукты_с_актуальными_ценами(тема_данные.get("продукты", []))
