...
def форматировать_пост(рерайт, оригинальный_заголовок, post_id=None):
    ...
    if post_id and оригинальный_заголовок:
...
def сохранить_пост_в_блог(текст, изображение_url, заголовок, источник='menshealth', расширенный_текст=None, все_изображения=None, post_id=None):
...
def главная():
    ...
    for i, статья in enumerate(релевантные[:максимальное_количество_попыток]):
        ...
        print("\n🔍 Проверка уникальности перед сохранением...")
        успех_сохранения = сохранить_пост_в_блог(рерайт_telegram, фото_url, статья['title'], 'menshealth', расширенный_текст, все_изображения, post_id=post_id)
        ...
        # Если контент уникален и сохранён, форматируем пост с ссылкой и отправляем в Telegram
        print("\n📤 Отправка в Telegram...")
        # Форматируем пост для Telegram с ссылкой на полную статью
        пост = форматировать_пост(рерайт_telegram, статья['title'], post_id=post_id)
