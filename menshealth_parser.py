...
9. Запрещено: бренды, товары, цены, промокоды, реклама
...
def сохранить_пост_в_блог(текст, изображение_url, заголовок, источник='menshealth', расширенный_текст=None, все_изображения=None, post_id=None, url_статьи=None, rss_feed_url=None):
    ...
    try:
        ...
        print(f"📝 Используется оригинальный заголовок: '{заголовок_русский}'")
...
def главная():
    ...
    for i, статья in enumerate(статьи_для_обработки):
        ...
        if публиковать_на_сайт:
            ...
            url_статьи = ''
            # ✅ ИСПРАВЛЕНИЕ: Используем оригинальный заголовок из спарсенной статьи
            заголовок_русский = адаптировать_заголовок_для_русской_аудитории(статья['title'], '')
