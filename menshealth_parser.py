...
def отправить_в_telegram(текст, фото_url=None):
    ...
    try:
        if фото_url:
            ...
            if len(текст) > 1024:
                # Обрезаем до 950 символов, чтобы с учётом добавленного текста было <= 1024
                продолжение = "\n\n... (продолжение: tabatatimer.ru)"
                max_len = 1024 - len(продолжение)
                оригинальная_длина = len(текст)
                текст = текст[:max_len] + продолжение
