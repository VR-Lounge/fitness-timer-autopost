...
def получить_последние_использованные_источники(n=4):
    """
    Получает последние n использованных источников (доменов) из blog-posts.json
    ...
    try:
        ...
        # Получаем домены из последних n постов
        for пост in посты[:n]:
...
def главная():
    ...
    print(f"🔄 Ротация источников: исключаем последние {len(последние_источники)} источников")
    ...
    for статья in все_статьи:
        ...
        if домен_источника in последние_источники:
            print(f"🔄 Пропущена статья из недавно использованного источника: {домен_источника}")
            не_релевантных += 1
            continue
    ...
    for i, статья in enumerate(статьи_для_обработки):
        ...
        if публиковать_на_сайт:
            ...
            print("\n🔍 Проверка уникальности перед сохранением в блог...")
            результат_сохранения = сохранить_пост_в_блог(рерайт_telegram, фото_url, статья['title'], 'menshealth', расширенный_текст, все_изображения, post_id=post_id, url_статьи=статья['link'])
