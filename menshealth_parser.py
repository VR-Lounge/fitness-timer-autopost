...
def сохранить_пост_в_блог(текст, изображение_url, заголовок, источник='menshealth', расширенный_текст=None, все_изображения=None, post_id=None):
    ...
    try:
        ...
        if все_изображения:
            print(f"📸 Обрабатываю {len(все_изображения)} изображений из RSS...")
            for idx, img_dict in enumerate(все_изображения[:10]):  # Ограничиваем до 10 изображений
                img_url = img_dict.get('url', '')
                if not img_url:
                    print(f"  ⚠️ Пропущено изображение {idx + 1}: нет URL")
                    continue
                
                # Скачиваем и загружаем в Yandex Cloud
                img_post_id = f"{post_id}_{idx}"
                print(f"  📥 Скачиваю изображение {idx + 1}/{len(все_изображения[:10])}: {img_url[:60]}...")
...
def главная():
    ...
    for i, статья in enumerate(релевантные[:максимальное_количество_попыток]):
        ...
        print("\n🔍 Проверка уникальности перед сохранением...")
        результат_сохранения = сохранить_пост_в_блог(рерайт_telegram, фото_url, статья['title'], 'menshealth', расширенный_текст, все_изображения, post_id=post_id)
        ...
        print("\n📤 Отправка в Telegram...")
        # Форматируем пост для Telegram с ссылкой на полную статью (используем русский заголовок)
        пост = форматировать_пост(рерайт_telegram, заголовок_русский, post_id=post_id, url=url_статьи)
