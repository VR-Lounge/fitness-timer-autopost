...
def форматировать_текст_для_html(текст, заголовок, теги):
    ...
    for блок in блоки:
        if блок['type'] == 'h3':
            ...
            результат.append(f"<ol>{элементы}</ol>")
...
def сгенерировать_html_страницу(пост):
    ...
    # Проверяем, нужно ли скачивать изображение (если оно ещё не из Yandex Cloud)
    if изображение_url and not изображение_url.startswith('https://www.tabatatimer.ru/images/blog/'):
        print(f"📥 Проверяю изображение: {изображение_url[:80]}...")
        локальное_изображение_url = скачать_и_загрузить_изображение(изображение_url, post_id)
        if локальное_изображение_url and локальное_изображение_url != изображение_url:
            изображение = локальное_изображение_url
            print(f"✅ Использую локальное изображение: {изображение}")
        else:
            изображение = изображение_url
    else:
        изображение = изображение_url
