...
def создать_галерею_изображений(изображения, заголовок, теги):
    ...
    - ВСЕ изображения (включая первое) показываются в слайдере с превьюшками
    - Если есть только одно изображение, галерея всё равно создаётся
    """
    if not изображения:
        return ''  # Если нет изображений, галерея не нужна
    
    # ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: ВСЕ изображения попадают в галерею
    # Первое изображение уже показано выше как главное, но оно также должно быть в галерее
    # Используем ВСЕ изображения для галереи
    все_изображения_для_галереи = изображения
    
    # Если нет изображений, не создаём галерею
    ...
    <div class="blog-post-gallery">
        <h3>Иллюстрации из статьи ({len(все_изображения_для_галереи)})</h3>
