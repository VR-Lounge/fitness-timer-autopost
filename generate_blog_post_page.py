...
def создать_галерею_изображений(изображения, заголовок, теги):
    ...
    # Если только одно изображение, всё равно создаём галерею (оно будет единственным)
    ...
    # ВАЖНО: Если есть только одно изображение, но оно не главное, показываем его в галерее
    # Или если есть несколько изображений, показываем все кроме первого
    if not дополнительные_изображения and len(изображения) == 1:
...
def форматировать_текст_для_html(текст, заголовок, теги):
    ...
    for строка in строки:
        ...
        # Обрабатываем markdown ссылки [текст](url) ПЕРЕД экранированием HTML
        # Это важно, чтобы ссылки правильно конвертировались в HTML
        def заменить_markdown_ссылку(match):
            текст_ссылки = match.group(1)
            url_ссылки = match.group(2)
            # Проверяем, не является ли это уже HTML ссылкой
            if url_ссылки.startswith('http://') or url_ссылки.startswith('https://') or url_ссылки.startswith('/'):
                return f'<a href="{url_ссылки}" target="_blank" rel="noopener noreferrer">{текст_ссылки}</a>'
            return match.group(0)  # Если не похоже на URL, оставляем как есть
        
        строка = re.sub(r'\[([^\]]+)\]\(([^)]+)\)', заменить_markdown_ссылку, строка)
        ...
        # Экранируем HTML, но сохраняем уже добавленные теги (включая ссылки)
...
def адаптировать_заголовок_для_русской_аудитории(заголовок, текст=''):
    ...
    # ВАЖНО: Избегаем жестко закодированных заголовков для предотвращения дубликатов
    # Вместо этого используем более уникальные варианты на основе контекста
    if 'hybrid athlete' in заголовок_нижний or ('гибридн' in текст_нижний and 'атлет' in текст_нижний):
        # Добавляем уникальность через дополнительные слова из текста
        if 'тренировочн' in текст_нижний or 'program' in заголовок_нижний:
            return 'Гибридный атлет: программа силы и выносливости'
        elif 'систем' in текст_нижний or 'system' in заголовок_нижний:
            return 'Гибридный атлет: система тренировок'
        else:
            return 'Гибридный атлет: сила + выносливость'
    elif re.search(r'strong.*conditioned.*ready', заголовок_нижний, re.IGNORECASE):
        return 'Сильный и выносливый: готов к любому вызову'
    elif 'сила' in текст_нижний and 'выносливость' in текст_нижний and 'сочет' in текст_нижний:
    ...
    if 'hybrid' in заголовок_нижний and ('athlete' in заголовок_нижний or 'тренировк' in текст_нижний):
        # Добавляем уникальность через контекст
        if 'program' in заголовок_нижний or 'тренировочн' in текст_нижний:
            return 'Гибридный атлет: программа тренировок'
        else:
            return 'Гибридный атлет: сила + выносливость'
        ...
        # Избегаем дубликатов - используем более уникальные варианты на основе контекста
        if 'до' in текст_нижний and 'после' in текст_нижний:
            return 'Питание до и после тренировки: что есть для лучших результатов'
        elif 'рецепт' in текст_нижний or 'блюд' in текст_нижний:
            return 'Рецепты для тренировок: питание для энергии и восстановления'
        elif 'белк' in текст_нижний or 'протеин' in текст_нижний:
            return 'Белковое питание для тренировок: основа мышечного роста'
        else:
            # Используем уникальный вариант на основе первых слов оригинального заголовка
            слова_заголовка = заголовок.split()[:3]
            if len(слова_заголовка) >= 2:
                return f"{' '.join(слова_заголовка)}: питание для тренировок"
    ...
    if 'strong' in заголовок_краткий.lower() and 'conditioned' in заголовок_краткий.lower():
        # Избегаем дубликатов - используем более уникальный вариант
        return 'Сила и выносливость: гибридный подход к тренировкам'
