    ...
    try:
        ...
        if все_изображения:
            ...
            for idx, img_dict in enumerate(изображения_для_обработки):
                ...
                # ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Всегда сохраняем изображение, даже если URL оригинальный
                if not локальное_img_url:
                    # Если всё равно нет URL, пропускаем это изображение
...
def главная():
    ...
    for i, статья in enumerate(статьи_для_обработки):
        ...
        if публиковать_на_сайт:
            ...
            результат_сохранения = сохранить_пост_в_блог(рерайт_telegram, фото_url, заголовок_русский_для_рерайта, 'womenshealth', расширенный_текст, все_изображения, post_id=post_id, url_статьи=статья['link'], rss_feed_url=статья.get('rss_feed_url', ''))
