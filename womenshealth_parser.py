...
def сохранить_пост_в_блог(текст, изображение_url, заголовок, источник='womenshealth', расширенный_текст=None):
    """Сохраняет пост в JSON файл для блога с проверкой уникальности
    
    Args:
        текст: короткий текст для Telegram (900 символов)
        изображение_url: URL изображения
...
def главная():
    ...
    for i, статья in enumerate(релевантные[:максимальное_количество_попыток]):
        ...
        print("🤖 Рерайтинг для Telegram через DeepSeek AI...")
        рерайт_telegram = рерайтить_через_deepseek(полный_текст, статья['title'])
        ...
        расширенный_текст = расширить_контент_для_статьи(полный_текст, статья['title'])
        ...
        print("\n🔍 Проверка уникальности перед сохранением...")
        успех_сохранения = сохранить_пост_в_блог(пост, фото_url, статья['title'], 'womenshealth', расширенный_текст)
