...
def расширить_контент_для_статьи(оригинальный_текст, заголовок):
    ...
    try:
        ...
        system_prompt = """Ты крутой фитнес-эксперт и копирайтер, который пишет полноценные статьи для блога о фитнесе, здоровье и тренировках для ДЕВУШЕК. Твой стиль - разговорный, но информативный, как с опытным тренером.
...
- ЖЕНСКИЕ "БОЛИ": деликатно закрывай их (недостаток времени, сложность начать, страх не справиться, желание похудеть, низкая мотивация) - но с хорошим настроением и поддержкой"""
        ...
        user_prompt = f"""Расширь и перепиши эту статью для полноценной статьи на сайте для ДЕВУШЕК (2000-4000 символов):
...
def сохранить_пост_в_блог(текст, изображение_url, заголовок, источник='womenshealth', расширенный_текст=None, все_изображения=None):
    ...
    try:
        ...
        if все_изображения:
            print(f"📸 Обрабатываю {len(все_изображения)} релевантных изображений...")
...
def главная():
    ...
    for i, статья in enumerate(релевантные[:максимальное_количество_попыток]):
        ...
        print("\n🔍 Проверка уникальности перед сохранением...")
        успех_сохранения = сохранить_пост_в_блог(пост, фото_url, статья['title'], 'womenshealth', расширенный_текст, все_изображения)
