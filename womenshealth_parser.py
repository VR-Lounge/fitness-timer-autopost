...
def сохранить_пост_в_блог(текст, изображение_url, заголовок, источник='womenshealth', расширенный_текст=None, все_изображения=None, post_id=None, url_статьи=None, rss_feed_url=None):
    ...
    try:
        ...
        # Если DeepSeek ничего не выбрал — берём первое ещё не использованное фото статьи
        from_article_fallback = False
        if not лучшее_изображение and все_изображения:
            for img_dict in все_изображения:
                img_url = (img_dict or {}).get('url', '')
                if not img_url:
                    continue
                normalized_url = img_url.split('?')[0].lower()
                используется = any(
                ...
                if not используется:
                    лучшее_изображение = dict(img_dict)
                    лучшее_изображение['is_main'] = True
                    from_article_fallback = True
                    print(f"✅ Fallback: первое уникальное фото статьи: {img_url[:80]}...")
                    break

        # Если не нашли подходящее изображение из статьи — берём из коллекций
        allow_stock_reuse = False
        if not лучшее_изображение:
            ...
            if из_коллекции:
                ...
                лучшее_изображение['is_main'] = True
                allow_stock_reuse = True  # коллекция сама может вернуть reuse при исчерпании
        ...
        if изображение_для_скачивания:
            ...
            if not is_skinnyms and not allow_stock_reuse:
                ...
                if используется:
                    print("⚠️ Фото уже в недавних постах — беру сток из коллекции...")
                    из_коллекции = получить_релевантное_изображение_для_статьи(
                        заголовок_русский,
                        текст_для_анализа,
                        теги,
                    ...
                    if из_коллекции and из_коллекции.get('url'):
                        изображение_для_скачивания = из_коллекции['url']
                        лучшее_изображение = dict(из_коллекции)
                        лучшее_изображение['is_main'] = True
                        allow_stock_reuse = True
                    else:
                        print("❌ Нет уникального изображения и сток недоступен")
                        return False
            
            # Получаем список использованных изображений для проверки уникальности
            if is_skinnyms or allow_stock_reuse or from_article_fallback or RECIPES_ONLY:
                # Сток / article-fallback / recipes: не блокируем второй проход DeepSeek
                все_использованные_urls = [] if (is_skinnyms or allow_stock_reuse) else получить_использованные_изображения_из_постов(существующие_посты)
...
def главная():
    ...
    # Для RECIPES_ONLY библиотека skinnyms (CF) бесполезна; RSS — основной путь.
    # source_filter=None: кандидаты из библиотеки с source recipes / nutrition RSS тоже ок.
    ...
    for i, статья in enumerate(статьи_для_обработки):
        ...
        if not расширенный_текст:
            ...
            if not is_skinnyms:
                ...
                if содержит_рекламные_маркеры(текст_для_фильтра):
                    if RECIPES_ONLY:
                        print("⚠️ Рекламные маркеры в расширенной статье — публикуем короткий Telegram-текст")
                        расширенный_текст = рерайт_telegram
                    else:
