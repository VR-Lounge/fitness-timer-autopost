...
def сохранить_пост_в_блог(текст, изображение_url, заголовок, источник='womenshealth', расширенный_текст=None, все_изображения=None, post_id=None, url_статьи=None):
...
def главная():
    ...
    for i, статья in enumerate(статьи_для_обработки):
        ...
        if публиковать_на_сайт:
            ...
            print("\n🔍 Проверка уникальности перед сохранением в блог...")
            результат_сохранения = сохранить_пост_в_блог(рерайт_telegram, фото_url, статья['title'], 'womenshealth', расширенный_текст, все_изображения, post_id=post_id, url_статьи=статья['link'])
