...
def главная():
    ...
    for i, статья in enumerate(статьи_для_обработки):
        ...
        рерайт_telegram = рерайтить_через_deepseek(parsed['content'], заголовок_для_рерайта)
        ...
        # ✅ ИСПРАВЛЕНИЕ: Используем оригинальный заголовок из спарсенной статьи
        заголовок_для_расширения = оригинальный_заголовок_статьи if 'оригинальный_заголовок_статьи' in locals() else статья['title']
        расширенный_текст = расширить_контент_для_статьи(parsed['content'], заголовок_для_расширения)
        ...
        if публиковать_на_сайт:
            ...
            if not результат_сохранения or (isinstance(результат_сохранения, dict) and not результат_сохранения.get('success')):
                ...
                if is_skinnyms:
                    ...
                    результат_сохранения = сохранить_пост_в_блог(рерайт_telegram, фото_url, заголовок_для_сохранения, 'womenshealth', расширенный_текст, все_изображения, post_id=post_id, url_статьи=статья['link'], rss_feed_url=статья.get('rss_feed_url', ''))
