...
def проверить_полную_уникальность(текст: str, image_url: str, существующие_посты: List[Dict] = None, заголовок: str = None, url_статьи: str = None) -> Tuple[bool, str]:
    ...
    ✅ УПРОЩЁННАЯ ВЕРСИЯ: Для skinnyms.com проверяем только URL источника
    ...
    """
    # ✅ УПРОЩЕНИЕ: Для skinnyms.com проверяем ТОЛЬКО URL источника
    if url_статьи and 'skinnyms.com' in url_статьи.lower():
        print("✅ Источник: skinnyms.com - проверяем только уникальность URL источника")
        
        # Проверяем только, не публиковали ли мы уже эту статью (по URL)
        if существующие_посты:
        ...
        # Для skinnyms.com считаем контент уникальным (пропускаем проверки изображений и текста)
        return True, "Контент из skinnyms.com уникален (проверен только URL источника)"
    
    # Для других источников - полная проверка (старая логика)
    # ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Для skinnyms.com пропускаем все проверки хешей
    # (но проверяем URL источника выше)
    
    # 1. Проверяем уникальность пары ФОТО+ТЕКСТ (только для не-skinnyms.com)
    уникальна_пара, хеш_пары = проверить_уникальность_пары(текст, image_url)
    if not уникальна_пара:
        # ✅ ЕСЛИ/ТО: Для skinnyms.com очищаем хеши и пропускаем
        if url_статьи and 'skinnyms.com' in url_статьи.lower():
            print(f"⚠️ Обнаружена блокировка пары ФОТО+ТЕКСТ для skinnyms.com (хеш: {хеш_пары[:16]}...)")
            очистить_все_хеши_для_skinnyms()
            return True, "Контент из skinnyms.com уникален (проверка пары пропущена после очистки хешей)"
        return False, f"Комбинация ФОТО+ТЕКСТ уже использовалась (хеш: {хеш_пары[:16]}...)"
    
    # 2. Проверяем уникальность текста отдельно (только для не-skinnyms.com)
    уникален_текст, хеш_текста = проверить_уникальность_текста(текст)
    if not уникален_текст:
        # ✅ ЕСЛИ/ТО: Для skinnyms.com очищаем хеши и пропускаем
        if url_статьи and 'skinnyms.com' in url_статьи.lower():
            print(f"⚠️ Обнаружена блокировка текста для skinnyms.com (хеш: {хеш_текста[:16]}...)")
            очистить_все_хеши_для_skinnyms()
            return True, "Контент из skinnyms.com уникален (проверка текста пропущена после очистки хешей)"
        return False, f"Текст уже использовался (хеш: {хеш_текста[:16]}...)"
    
    # 3. Проверяем уникальность изображения отдельно
    # ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Для skinnyms.com очищаем хеши если обнаружена блокировка
    уникально_изображение, хеш_изображения = проверить_уникальность_изображения(image_url)
    if not уникально_изображение:
        # ✅ ЕСЛИ/ТО: Если это skinnyms.com и изображение заблокировано - очищаем все хеши
        if url_статьи and 'skinnyms.com' in url_статьи.lower():
            print(f"⚠️ Обнаружена блокировка изображения для skinnyms.com (хеш: {хеш_изображения[:16]}...)")
            print("✅ Очищаю все хеши для skinnyms.com (изображения на самом деле ещё не использовались)")
            очистить_все_хеши_для_skinnyms()
            # После очистки проверяем снова - теперь должно пройти
            уникально_изображение, _ = проверить_уникальность_изображения(image_url)
            if уникально_изображение:
                print("✅ После очистки хешей изображение теперь уникально!")
            else:
                # Если всё ещё не уникально - это странно, но пропускаем проверку для skinnyms.com
                print("⚠️ Изображение всё ещё в списке, но для skinnyms.com пропускаем проверку")
                return True, "Контент из skinnyms.com уникален (проверка хешей пропущена)"
        else:
    ...
    # ✅ Для skinnyms.com эта проверка уже выполнена выше, пропускаем
    if url_статьи and 'skinnyms.com' not in url_статьи.lower() and существующие_посты:
    ...
    # ✅ Для skinnyms.com пропускаем семантическую проверку (статьи считаются 100% релевантными)
    if url_статьи and 'skinnyms.com' not in url_статьи.lower() and существующие_посты and DEEPSEEK_API_KEY:
...
def очистить_все_хеши_для_skinnyms():
    """
    ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Очищает ВСЕ хеши для skinnyms.com
    
    Вызывается когда обнаруживается блокировка по хешам для skinnyms.com
    (эти изображения на самом деле ещё не использовались)
    """
    данные = загрузить_хеши_контента()
    
    # Удаляем все хеши изображений, которые могут быть из skinnyms.com
    # (проверяем по URL - если URL содержит skinnyms.com, удаляем соответствующие хеши)
    оригинальные_хеши = данные.get('image_hashes', []).copy()
    очищенные_хеши = []
    
    # ВАЖНО: Мы не можем точно определить какие хеши от skinnyms.com,
    # поэтому очищаем ВСЕ хеши изображений для полной уверенности
    print("⚠️ ОЧИСТКА: Удаляю все хеши изображений для skinnyms.com...")
    print(f"   Было хешей изображений: {len(данные.get('image_hashes', []))}")
    
    # Очищаем все хеши изображений
    данные['image_hashes'] = []
    данные['content_pairs'] = []  # Также очищаем пары ФОТО+ТЕКСТ
    
    сохранить_хеши_контента(данные)
    print(f"✅ Хеши очищены! Теперь: {len(данные.get('image_hashes', []))} хешей изображений")

def сохранить_контент_как_использованный(текст: str, image_url: str, url_статьи: str = None):
    """
    Сохраняет контент как использованный (добавляет хеши)
    
    ✅ УПРОЩЕНИЕ: Для skinnyms.com НЕ сохраняем хеши (чтобы не блокировать повторное использование изображений)
    """
    # ✅ Для skinnyms.com не сохраняем хеши (статьи считаются 100% релевантными)
    if url_статьи and 'skinnyms.com' in url_статьи.lower():
        print("✅ Источник: skinnyms.com - хеши не сохраняются (изображения могут использоваться повторно)")
        return
