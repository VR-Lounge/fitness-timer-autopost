...
def скачать_и_загрузить_изображение(url, post_id=None):
    """
    ...
    if not локальный_путь:
        print(f"⚠️ Не удалось скачать изображение, используем оригинальный URL")
        return url
    ...
    if успех:
        ...
        # Если не удалось загрузить в Yandex Cloud, возвращаем локальный путь
        # (в production это должно быть yandex_url, но для тестирования используем локальный)
        return yandex_url
